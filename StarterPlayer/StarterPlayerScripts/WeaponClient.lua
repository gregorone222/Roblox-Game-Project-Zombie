...
local PlayLocalGunshotSoundEvent = RemoteEvents:WaitForChild("PlayLocalGunshotSoundEvent")
local PlayGunshotSoundEvent = RemoteEvents:WaitForChild("PlayGunshotSoundEvent")
...
-- Audio suppression functions
local function suppressGunshotSoundsTemporarily()
	suppressGunshotSounds = true
	-- Clear any existing 3D gunshot sounds
	for _, soundPart in pairs(workspace:GetChildren()) do
		if soundPart.Name == "GunshotSound" and soundPart:IsA("BasePart") then
	...
	-- Re-enable after a short delay to ensure local sound plays without interference
	task.delay(0.1, function()
		suppressGunshotSounds = false
	end)
end

local function setupGunshotSoundSuppressor()
	-- Continuously monitor for and suppress unwanted 3D gunshot sounds
	workspace.ChildAdded:Connect(function(child)
		if suppressGunshotSounds and child.Name == "GunshotSound" and child:IsA("BasePart") then
...
local function onStepped(dt)
	...
	if not UserInputService.TouchEnabled and isMouseDown and canShoot and not reloading and not isKnocked then
		...
		-- REMOVED: AudioManager.playSound(weaponStats.Sounds.Fire, currentWeapon) -- Now handled server-side
...
PlayLocalGunshotSoundEvent.OnClientEvent:Connect(function(weaponName)
	-- Only play the sound if we're currently holding this weapon
	if currentWeapon and weaponName == weaponName then
		local weaponStats = WeaponModule.Weapons[weaponName]
		if weaponStats and weaponStats.Sounds and weaponStats.Sounds.Fire then
			-- Play the gunshot sound locally for immediate, clear audio
			AudioManager.playSound(weaponStats.Sounds.Fire, currentWeapon)
		end
	end
end)

PlayGunshotSoundEvent.OnClientEvent:Connect(function(shooter, position, weaponName)
	if shooter == player then return end -- Ignore our own shots (handled by PlayLocalGunshotSoundEvent)
	local weaponStats = WeaponModule.Weapons[weaponName]
	if weaponStats and weaponStats.Sounds and weaponStats.Sounds.Fire then
		AudioManager.play3DSound(weaponStats.Sounds.Fire, position)
	end
end)
...
-- Initialize gunshot sound suppressor
setupGunshotSoundSuppressor()
