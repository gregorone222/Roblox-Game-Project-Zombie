...
function BoosterManager.GetBoosterData(player)
	...
	-- Validasi dan perbaikan data yang ada
	if not playerData.data.boosters then
		playerData.data.boosters = { Owned = {}, Active = {} }
	end

	-- Pastikan sub-tabel penting ada untuk mencegah error pada data lama/rusak
	if not playerData.data.boosters.Owned then
		playerData.data.boosters.Owned = {}
	end
	if not playerData.data.boosters.Active then
		playerData.data.boosters.Active = {}
	end
