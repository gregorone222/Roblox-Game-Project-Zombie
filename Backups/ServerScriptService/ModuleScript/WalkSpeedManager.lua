...
updateEvent.OnServerEvent:Connect(function(player, modifierName, value)
	...
	local allowedModifiers = { sprint = true, aim = true } -- RELOAD is now server-authoritative
	if not allowedModifiers[modifierName] then
		return
	end

	if modifierName == "aim" then
		if value == true then
	...
	elseif modifierName == "sprint" then
		if value then
			-- Fix: Server-authoritative sprint speed (ignoring client value)
			WalkSpeedManager.add_modifier(player, modifierName, 6)
		else
...
-- Server-side authoritative check for reload status
RunService.Heartbeat:Connect(function()
	for _, player in ipairs(Players:GetPlayers()) do
		local char = player.Character
		if char and char:FindFirstChild("Humanoid") then
			local isReloading = char:GetAttribute("IsReloading") == true
			local modifiers = playerModifiers[player.UserId]
			local hasReloadModifier = modifiers and modifiers.reload

			if isReloading and not hasReloadModifier then
				WalkSpeedManager.add_modifier(player, "reload", RELOAD_MODIFIER)
			elseif not isReloading and hasReloadModifier then
				WalkSpeedManager.remove_modifier(player, "reload")
			end
		end
	end
end)
