		...
		for id, data in pairs(missions) do
			if not data.Completed then
				...
				if config and config.Type == eventType and (not config.WeaponType or config.WeaponType == weaponType) then
					...
					missionProgressUpdatedEvent:FireClient(player, {missionID=id, newProgress=data.Progress, completed=data.Completed, justCompleted=data.Completed})
				end
			end
		end
	end
...
function MissionManager:RerollWeeklyMission(player, missionIDToReroll)
	local missionsData = self.GetData(player)
	if missionsData.Weekly.RerollUsed then
		return false, "Anda sudah menggunakan reroll mingguan."
	end

	if not missionsData.Weekly.Missions[missionIDToReroll] then
		return false, "Misi yang dipilih tidak valid."
	end

	-- Hapus misi lama
	missionsData.Weekly.Missions[missionIDToReroll] = nil

	-- Dapatkan misi baru yang unik
	local available = {}
	local currentIDs = {}
	for id, _ in pairs(missionsData.Weekly.Missions) do
		table.insert(currentIDs, id)
	end

	for _, m in ipairs(MissionConfig.WeeklyMissions) do
		if not table.find(currentIDs, m.ID) and not table.find(missionsData.RecentMissions, m.ID) then
			table.insert(available, m)
		end
	end
	...
	local newMission = available[math.random(#available)]
	missionsData.Weekly.Missions[newMission.ID] = { Progress = 0, Completed = false, Claimed = false }
	table.insert(missionsData.RecentMissions, newMission.ID)

	missionsData.Weekly.RerollUsed = true
	self.SaveData(player, missionsData)
