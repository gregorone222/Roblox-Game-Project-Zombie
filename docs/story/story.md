# 📜 Story & Lore

Panduan narasi, profil karakter, dan plot cerita episodik untuk **Zombie?**

---

## 🎭 Narrative Philosophy

### Core Philosophy: "Memory over Addiction"
Menciptakan pengalaman Co-op yang **hangat dan bermakna**, bukan grinding tanpa akhir.

### Tone of Narrative
Meskipun visualnya kartun/stylized, cerita harus **EMOSIONAL & MELANKOLIS**.

| Element | Description |
|:--------|:------------|
| **Kontras** | Warna dunia cerah, tapi catatan menceritakan kehilangan |
| **Environmental Storytelling** | Boneka ditinggalkan, meja makan belum selesai, surat terakhir |
| **Inspirasi** | Adventure Time (Simon arc), Gravity Falls (Weirdmageddon) |

> **"Kartun di luar, berat di dalam."**

### Inspirasi Tone (Penjelasan)

#### Adventure Time - Simon Arc
Simon Petrikov adalah manusia normal yang menemukan mahkota ajaib. Mahkota itu perlahan mengambil alih pikirannya dan mengubahnya menjadi Ice King - villain konyol yang tidak ingat siapa dirinya.
- **Visual:** Ice King kelihatan lucu, absurd, kartun
- **Story:** Backstory-nya SANGAT SEDIH - kehilangan tunangan, kehilangan ingatan
- **Pelajaran:** Kontras antara tampilan vs kedalaman emosional

#### Gravity Falls - Weirdmageddon
Arc akhir dimana Bill Cipher mengambil alih dunia. Kartun anak-anak dengan:
- Stakes nyata (karakter bisa mati)
- Tone gelap meskipun visual colorful
- Pengorbanan dan konsekuensi permanen

**Intinya:** Kartun bisa punya emotional depth tanpa kehilangan visual ramah anak.

### Tone Balance Examples
```
VISUAL: Zombie Runner berlari dengan animasi lucu, warna vibrant
AUDIO: Tapi jika didengar... dia menangis saat berlari

VISUAL: Boneka teddy di swing terlihat cute  
STORY: Jika di-inspect: "Seseorang sangat menyayangi pemilik boneka ini"

VISUAL: Boss Hive Mother terlihat mengerikan tapi kartun
LORE: Dia dulunya ibu yang ingin melindungi anak-anaknya dari wabah
```

---

## 📋 Content Rules

Aturan konten yang HARUS diikuti dalam pengembangan game:

| Rule | Description |
|:-----|:------------|
| **No Music** | Game tidak menggunakan musik. Hanya SFX dan ambient sounds. |
| **No Adult Women** | Tidak menampilkan karakter perempuan dewasa. Anak-anak perempuan boleh. |
| **No Pay-to-Win** | Tidak ada pembelian yang memberikan advantage gameplay. |
| **Cosmetic Gacha** | Gacha hanya untuk skin/visual. TIDAK ada stat atau efek gameplay. |
| **No Horror Gore** | Visual stylized, tidak ada darah realistis atau gore. |

### ⚠️ Visual Consistency Reminder

> **PENTING: Visual HARUS selalu STYLIZED/CARTOON meskipun lore-nya berat.**

| Lore (Berat) | Visual (Ringan) |
|:-------------|:----------------|
| Hive Mother = ibu kehilangan anak | Monster cartoon vibrant, tidak ada fitur manusia |
| Lab eksperimen | Ruangan colorful dengan peralatan stylized |
| Zombie = manusia terinfeksi | Desain lucu/quirky, bukan horror realistis |

Referensi visual: **Fortnite, Overwatch, Gravity Falls** - bukan The Last of Us realistis.

### Monetization Philosophy
```
✅ ALLOWED:
- Skin senjata (visual only)
- Character cosmetics
- Emotes
- Titles

❌ NOT ALLOWED:
- Damage boost
- Health boost
- Exclusive weapons dengan stats berbeda
- Pay-to-skip content
```

---

## 🎯 The Title Mystery: Why "Zombie?"

### Plot Twist Teaser

> **Kenapa judulnya "Zombie?" dengan tanda tanya?**

Sepanjang ACT 1-3, pemain percaya wabah zombie hanyalah bencana alam atau kecelakaan. Tapi petunjuk-petunjuk kecil mengarah ke sesuatu yang lebih gelap:

**Hints di ACT 1-2:**
- Lab notes dengan logo korporasi misterius
- Internal memo: *"Lanjutkan testing meskipun ada gejala..."*
- Dokumen terhapus tentang "Project Genesis"

**Hints di ACT 3-4:**
- Ditemukan bunker dengan fasilitas mewah
- Whistleblower letter yang tidak pernah terkirim
- Alexander: *"Ini bukan kecelakaan... seseorang menyebabkan ini."*

---

## 🏢 The Conspiracy: Zenith Pharmaceuticals

### The Corporation

| Aspek | Detail |
|:------|:-------|
| **Nama** | Zenith Pharmaceuticals |
| **Bidang** | Pharmaceutical & Biotech |
| **Slogan** | "Advancing Human Potential" |
| **CEO** | Dr. Heinrich Kessler |
| **HQ** | Kota metropolitan (ACT 2 setting) |

### What Happened

> **Zenith mengembangkan "Genesis Serum" - eksperimen enhancement manusia untuk dijual ke militer.**

Serum gagal dan menyebabkan mutasi. Alih-alih menghentikan dan melaporkan, Zenith:
- Menyembunyikan bukti
- Melanjutkan testing di desa terpencil (ACT 1)
- Menyiapkan bunker mewah untuk eksekutif

**Zombie adalah korban. Zenith adalah penjahat.**

### The Truth (ACT 5 Reveal)

> **Wabah ini bukan kecelakaan. Ini corporate negligence.**

**Emotional Weight:**
- Rage terhadap sistem yang korup
- *"Mereka TAHU dan tetap lanjut..."*
- Alexander: *"Berapa nyawa yang mereka korbankan demi profit?"*

### Key Evidence (Collectibles)

| Item | Location | Content |
|:-----|:---------|:--------|
| **Internal Memo** | ACT 1 Lab | "Subject showing symptoms. Proceed anyway." |
| **Whistleblower Letter** | ACT 3 | "I tried to stop them... they silenced me." |
| **Board Meeting Recording** | ACT 5 | "Acceptable losses for market potential." |
| **CEO Video** | ACT 7 | Old interview: "We care about humanity." (ironic) |
| **Evacuation Order** | ACT 8 | "Priority: Executive extraction only." |

---

## 👥 Characters

> 📎 **Detail lengkap:** [Characters Documentation](../entities/characters.md)

### Lobby NPCs (ACT 1)

| Character | Role | Location |
|:----------|:-----|:---------|
| **Alexander** | Komandan, Narrator | Gazebo |
| **Quartermaster** | Weapon/Equipment Seller | Barn |

*Saran NPC tambahan ada di dokumen characters.*

---

## 🏠 Lobby Lore: The Abandoned Farm

### Backstory
Farm ini dulunya milik **Keluarga Miller** - petani yang selamat di awal wabah. Mereka bertahan selama beberapa bulan sebelum... menghilang. Alexander dan kelompoknya menemukan farm ini kosong, tapi tertata rapi. Seakan pemiliknya berharap untuk kembali.

### Environmental Storytelling Props

| Prop | Location | Story Snippet |
|:-----|:---------|:--------------|
| **Family Photo** | Mantel perapian | Foto keluarga 4 orang (wajah blur) dengan tulisan "Miller Family, Summer" |
| **Child's Drawing** | Dinding dapur | Gambar crayon: rumah + matahari + 4 stick figure. Tulisan: "Our Home" |
| **Unfinished Dinner** | Meja makan | 4 piring dengan makanan berjamur. Kursi masih rapi. |
| **Last Letter** | Meja kerja | *"Jika ada yang menemukan ini... tolong urus Bella (boneka teddy). Sarah sangat menyayanginya."* |
| **Empty Swing** | Halaman depan | Ayunan bergerak pelan tertiup angin. Tidak ada yang mendudukinya. |
| **Abandoned Teddy** | Dekat swing | Boneka beruang bernama "Bella", kotor tapi utuh |
| **Broken Bicycle** | Halaman samping | Sepeda anak, patah, roboh di rumput |
| **Old Pickup Truck** | Belakang barn | Truk berkarat dengan ivy. Kunci masih di ignition. |
| **Memorial Board** | Dekat Leaderboard | Foto-foto survivor yang gugur. Diupdate setiap season. |

---

## 📖 Episodic Campaign

### Timeline Overview
```
[LOBBY] Farmhouse Basecamp
    ↓
┌─── ARC 1: THE OUTBREAK ───┐
│ [ACT 1] Cursed Village    │
│ [ACT 2] Silent City       │
│ [ACT 3] Underground       │
└───────────────────────────┘
    ↓
┌─── ARC 2: THE CONSPIRACY ─┐
│ [ACT 4] Zenith Complex    │
│ [ACT 5] Cover-Up ★TWIST   │
│ [ACT 6] Betrayal          │
└───────────────────────────┘
    ↓
┌─── ARC 3: THE RECKONING ──┐
│ [ACT 7] The Spread        │
│ [ACT 8] The Alliance      │
│ [ACT 9] The Reckoning     │
└───────────────────────────┘
    ↓
[ACT 10] The New Dawn - Healing Begins
```

---

## 🏚️ ACT 1: The Cursed Village

**Setting:** Desa terpencil yang misterius  
**Theme:** Survival Horror, Mystery  
**Boss Count:** 3

### Story Summary
Sinyal darurat terdeteksi dari desa terpencil. Tim Survivor dikirim untuk investigasi dan evakuasi. Mereka menemukan desa sudah dikuasai zombie - tapi **tidak ada petunjuk penyebabnya**. Fokus: bertahan hidup.

> **NOTE:** ACT 1 sengaja tidak memberikan petunjuk tentang Zenith. Player hanya tahu ada wabah, bukan siapa penyebabnya.

### Wave-by-Wave Story Beats

| Wave | Event Type | Story Content |
|:-----|:-----------|:--------------|
| **1** | Radio Transmission | *"Sinyal darurat terdeteksi. Evakuasi kemungkinan gagal. Hati-hati, Survivor."* |
| **5** | Discovery | Surat terakhir warga: *"Mereka sakit sejak minggu lalu... aku tidak tahu apa yang terjadi."* |
| **8** | Objective Wave | **Scavenge Mission:** Mengumpulkan Gas Canisters untuk generator |
| **10** | **BOSS WAVE** | **Plague Titan** muncul. |
| **12** | Audio Log | *"Siapapun yang menemukan ini... tolong beritahu keluargaku..."* |
| **15** | Environmental | Menemukan bunker warga yang terkunci (tidak bisa masuk). |
| **20** | Radio | Alexander: *"Tidak ada survivor. Tapi bagaimana wabah ini dimulai?"* |
| **22** | Objective Wave | **Defend Mission:** Mempertahankan Radio Tower |
| **25** | Discovery | Foto keluarga, mainan anak-anak - kenangan yang ditinggalkan. |
| **30** | **BOSS WAVE** | **The Hive Mother** muncul. |
| **35** | Audio Log | *"Aku dengar suara dari hutan... seperti ada yang memanggil..."* |
| **38** | Objective Wave | **Retrieve Mission:** Mengambil catatan medis dari klinik desa |
| **45** | Radio | Alexander: *"Ada sesuatu yang aneh di sini. Kita perlu bukti lebih."* |
| **50** | **FINAL BOSS** | **The Blighted Alchemist** muncul. |

> **End of ACT 1:** Player selesai tapi masih **tidak tahu penyebab wabah**. Pertanyaan terbuka untuk ACT 2-3.

### Boss Profiles

#### Boss 1: Plague Titan (Wave 10-15)
**True Identity:** Dr. Marcus Webb, Lead Scientist  
**HP:** 75,000  
**Signature:** Radiation Aura, Corrosive Slam

**Lore:**
> Marcus percaya eksperimennya akan menyelamatkan dunia. Ketika virus bermutasi, dia menyuntikkan dirinya dengan serum prototipe. Tubuhnya berubah, pikirannya sudah lama mati.

**Death Quote:** *"I was... so close... to saving... them..."*

---

#### Boss 2: The Hive Mother (Wave 30-35)
**True Identity:** Dr. Elena Voss, Chief Biologist  
**HP:** 100,000  
**Signature:** Spawn Larva, Acid Spit, Toxic Cloud

**Lore:**
> Elena kehilangan dua anaknya dalam outbreak awal. Dalam keputusasaan, dia mencoba "evolusi paksa". Hasilnya? Dia menjadi sarang untuk "anak-anak" terakhirnya.

**Death Quote:** *"My children... I just wanted... to protect them..."*

> ⚠️ **Note:** Visual in-game adalah monster tanpa fitur manusia yang jelas (sesuai content rules).

---

#### Boss 3: The Blighted Alchemist (Wave 50)
**True Identity:** Victor Crane, Project Director  
**HP:** 125,000  
**Signature:** Syringe Volley, Plague Bomb, Formula Switching

**Lore:**
> Victor terobsesi dengan "evolusi manusia sempurna". Dia menggunakan laboratorium desa untuk eksperimen ilegal. Baginya, wabah adalah "langkah pertama menuju spesies yang lebih kuat."

**Death Quote:** *"You think this ends here? The research... will continue..."*

**Post-Death:** Koordinat mengarah ke kota besar - **The Silent City**.

---

## 🌆 ACT 2: The Silent City (Coming Soon)

**Setting:** Kota metropolitan yang sekarang sunyi  
**Theme:** Urban survival, first doubts about "killing zombies"

**Story Hints:**
- Boss baru berbasis mutasi lanjutan
- Ditemukan dokumen Zenith yang tersembunyi
- Alexander: *"Ini bukan kebetulan... ada yang menyebabkan semua ini."*

---

## 💊 ACT 3: The Underground (Concept)

**Setting:** Bunker network bawah tanah  
**Theme:** Survivors, secrets, whistleblower evidence  

**The Discovery:**
> Bukti pertama bahwa Zenith terlibat langsung.

**Gameplay Features:**
- Underground survival mechanics
- Meet hidden survivor community
- Collect whistleblower documents

---

## 💔 Emotional Mechanics

### Memorial Collectibles
- **Photo Fragments** - Unlock gallery di Lobby
- **Audio Logs** - Suara dari masa lalu
- **Personal Items** - Toys, letters, journals

### Last Words
Zombie mati kadang bergumam: *"...home..."*, *"...sorry..."*, *"...love you..."*

### Quiet Moments
Setelah wave besar (10, 20, 30, 40), ada jeda 30 detik dengan ambient sounds dan Alexander berbicara.

### Rescue Events
Kadang ada NPC survivor terperangkap yang bisa diselamatkan.

---

## � Mystery & Secrets System

Terinspirasi dari Gravity Falls, game ini memiliki **layer rahasia** yang mendorong pemain untuk explore, decode, dan berdiskusi.

> *"I miss being obsessed with the show, reading every comic, decoding all the code, finding every secret..."*  
> — Gravity Falls fan

### Cipher System

Hidden text di game menggunakan cipher sederhana yang bisa di-decode:

| Cipher | Description | Example |
|:-------|:------------|:--------|
| **Caesar Shift** | Huruf digeser 3 posisi | "KHOS" = "HELP" |
| **A1Z26** | Huruf = angka (A=1, Z=26) | "8-5-12-16" = "HELP" |
| **Reverse** | Teks dibalik | "PLEH" = "HELP" |
| **Atbash** | A↔Z, B↔Y swap | "SVOK" = "HELP" |

### Hidden Messages per ACT

| ACT | Hidden Message | Decoded Meaning |
|:----|:---------------|:----------------|
| **1** | Lab wall: "CHQLWK GLG WKLV" | "ZENITH DID THIS" (Caesar +3) |
| **2** | Poster: "XLEVH FI GSV GIFGS" | "COVER UP THE TRUTH" (Atbash) |
| **3** | Document: "20-8-5-25 11-14-5-23" | "THEY KNEW" (A1Z26) |
| **4** | Server room: "SISENEGDETCHAP" | "PATCHED GENESIS" (Reverse) |
| **5** | Evidence vault: "SURMLW VQ GHQLVHG" | "PROFITS IN DENIED" (Caesar +3) |

### Environmental Secrets

- **Pattern Numbers**: Koordinat/tanggal yang mengarah ke sesuatu
- **NPC Name Anagrams**: Nama karakter adalah anagram bermakna
- **Audio Reversed**: Background sounds yang kalau reversed = clue
- **Visual Patterns**: Simbol berulang di environment

### In-Game Discovery

- **Lobby Gallery**: Decoded secrets unlock lore entries
- **Achievement Badges**: Rewards untuk pemain yang decode cipher
- **Hidden Rooms**: Beberapa area hanya bisa diakses setelah decode clue

---

## 📚 10 ACT Roadmap

### Arc Structure (3-3-3-1)

```
┌─────────────────────────────────────────────────────────────────┐
│  ARC 1: THE OUTBREAK (ACT 1-3)                                  │
│  Discovery → Urban Spread → Underground Secrets                  │
├─────────────────────────────────────────────────────────────────┤
│  ARC 2: THE CONSPIRACY (ACT 4-6)                                │
│  Zenith exposed → Government cover-up → Betrayal                 │
├─────────────────────────────────────────────────────────────────┤
│  ARC 3: THE RECKONING (ACT 7-9)                                 │
│  Global outbreak → Alliance formed → Final assault               │
├─────────────────────────────────────────────────────────────────┤
│  FINALE: THE NEW DAWN (ACT 10)                                  │
│  Justice served - World begins healing                           │
└─────────────────────────────────────────────────────────────────┘
```

### ACT Overview Table

| ACT | Title | Setting | Theme | Revelation | Twist/Climax |
|:----|:------|:--------|:------|:-----------|:-------------|
| **1** | The Cursed Village | Desa terpencil | Survival Horror | "Tidak ada petunjuk" | Boss: Blighted Alchemist |
| **2** | The Silent City | Kota metropolitan | Urban nightmare | "Logo aneh ditemukan" | Petunjuk pertama |
| **3** | The Underground | Bunker network | Secrets revealed | **"Lab Zenith ditemukan"** | Mulai curiga |
| **4** | The Corporation | Zenith HQ | Follow the money | "Internal memo leaked" | Zenith confirmed |
| **5** | The Cover-Up | Government facility | Conspiracy depth | **"MEREKA TAHU DARI AWAL"** | **MAJOR TWIST** |
| **6** | The Betrayal | Safe zone | Trust broken | "Ada spy di antara kita" | **ARC 2 CLIMAX** |
| **7** | The Spread | Multi-location | Global outbreak | "Zenith punya cabang global" | International stakes |
| **8** | The Alliance | Resistance base | Unite factions | "Kita tidak sendirian" | Coalition formed |
| **9** | The Reckoning | Zenith bunker | Justice | "CEO masih hidup" | **ARC 3 CLIMAX** |
| **10** | The New Dawn | Aftermath | Healing begins | "Dunia mulai pulih" | **SATISFYING ENDING** |

### 🗺️ Map Overview per ACT

| ACT | Map Name | Biome/Type | Visual Style | Key Landmarks |
|:----|:---------|:-----------|:-------------|:--------------|
| **1** | Cursed Village | Rural/Forest | Abandoned countryside, overgrown | Radio Tower, Ruined Houses, Lab Basement |
| **2** | Silent City | Urban | Empty metropolis, cars abandoned | Zenith HQ Tower, Hospital, Subway |
| **3** | Underground Bunker | Subterranean | Concrete tunnels, emergency lights | Survivor Camp, Armory, Water Treatment |
| **4** | Zenith Complex | Industrial | Sterile labs, corporate offices | Board Room, Research Labs, Server Room |
| **5** | Government Facility | Military | Fenced compound, guard towers | Command Center, Quarantine Zone, Evidence Vault |
| **6** | Safe Haven | Mixed | Fortified community, makeshift walls | Town Hall, Barracks, Radio Station |
| **7** | Port District | Coastal/Industrial | Shipping containers, docks | Cargo Ship, Warehouse, Lighthouse |
| **8** | Resistance Base | Mountain/Forest | Hidden camp, natural caves | War Room, Training Ground, Armory Cave |
| **9** | Zenith Bunker | Luxury Underground | Opulent shelter, marble & gold | CEO Suite, Escape Tunnel, Control Room |
| **10** | Reclaimed City | Urban/Restored | Signs of rebuilding, gardens | Memorial Plaza, New Hospital, Sunrise Point |

### Arc Details

#### ARC 1: THE OUTBREAK (ACT 1-3)
**Theme:** Discovery and growing suspicion

- **ACT 1:** Player fights zombies, finds mysterious lab
- **ACT 2:** Zenith logo discovered, first corporate hints
- **ACT 3:** Whistleblower evidence found in bunker

**Emotional Arc:** Confident → Curious → Suspicious

---

#### ARC 2: THE ORGANIZATION (ACT 4-6)
**Theme:** Conspiracy and betrayal

- **ACT 4:** Discover corporate involvement (Pharma? Government?)
- **ACT 5:** Meet hidden survivor communities
- **ACT 6:** Someone betrays the group (NPC twist)

**Emotional Arc:** Determined → Paranoid → Heartbroken

---

#### ARC 3: THE GLOBAL CRISIS (ACT 7-9)
**Theme:** Unity and sacrifice

- **ACT 7:** Outbreak goes worldwide
- **ACT 8:** Unite different survivor factions
- **ACT 9:** Final assault on source

**Emotional Arc:** Overwhelmed → Hopeful → Desperate

---

#### FINALE: ACT 10 - THE NEW DAWN
**Theme:** Justice and healing

- Zenith leadership defeated
- Evidence broadcast worldwide
- World begins recovery
- **Bittersweet but hopeful ending**

> **Philosophy:** Ending harus satisfying. Tidak cliffhanger. Player merasa perjalanan mereka bermakna.

### Boss Count Target

| ACT | Boss Count | Notes |
|:----|:-----------|:------|
| 1 | 3 | Plague Titan, Hive Mother, Blighted Alchemist |
| 2 | 3 | TBD |
| 3 | 2 | Focus on survival, less combat |
| 4-6 | 2-3 each | Escalating difficulty |
| 7-9 | 3 each | Global-scale threats |
| 10 | 1 | Final boss - meaningful conclusion |

**Total Target:** ~25-30 unique bosses across all ACTs

---

## �📝 Implementation Checklist

- [ ] Environmental props di Lobby
- [ ] Expand DialogueConfig
- [ ] Readable notes system
- [ ] Wave story triggers
- [ ] Boss death quotes
- [ ] Audio log collectibles
- [ ] Memorial collectible system
- [ ] Last Words voice lines
- [ ] Cipher/hidden text system
- [ ] ACT 2 development
- [ ] ACT 5 Zenith conspiracy reveal

---

*"In the end, it's not about surviving. It's about remembering why we survive."*  
— Alexander

